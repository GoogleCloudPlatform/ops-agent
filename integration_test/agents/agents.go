...
func RunOpsAgentDiagnostics(ctx context.Context, logger *logging.DirectoryLogger, vm *gce.VM) {
	...
	if isUAPPlugin {
		gce.RunRemotely(ctx, logger.ToFile("status_for_ops_agent_uap_plugin.txt"), vm, fmt.Sprintf("grpcurl -plaintext -max-time 60 -d '{}' localhost:%s plugin_comm.GuestAgentPlugin/GetStatus", OpsAgentPluginServerPort))
	} else {
...
func runOpsAgentDiagnosticsWindows(ctx context.Context, logger *logging.DirectoryLogger, vm *gce.VM) {
	...
	if gce.IsOpsAgentUAPPlugin() {
		...
		gce.RunRemotely(ctx, logger.ToFile("status_for_ops_agent_uap_plugin.txt"), vm, fmt.Sprintf(`C:\grpcurl.exe -plaintext -max-time 60 -d '{}' localhost:%s plugin_comm.GuestAgentPlugin/GetStatus`, OpsAgentPluginServerPort))
...
func getRestartOpsAgentCmd(imageSpec string) string {
	if gce.IsOpsAgentUAPPlugin() {
		...
		if gce.IsWindows(imageSpec) {
			...
			return fmt.Sprintf("%s -plaintext -max-time 60 -d '{}' localhost:%s plugin_comm.GuestAgentPlugin/Stop; Start-Sleep -Seconds 5; %s -plaintext -max-time 60 -d '{}' localhost:%s plugin_comm.GuestAgentPlugin/Start", grpcurlExecutable, OpsAgentPluginServerPort, grpcurlExecutable, OpsAgentPluginServerPort)
		}
		return fmt.Sprintf("%s -plaintext -max-time 60 -d '{}' localhost:%s plugin_comm.GuestAgentPlugin/Stop && sleep 5 && %s -plaintext -max-time 60 -d '{}' localhost:%s plugin_comm.GuestAgentPlugin/Start", grpcurlExecutable, OpsAgentPluginServerPort, grpcurlExecutable, OpsAgentPluginServerPort)
	}
...
func StartCommandForImage(imageSpec string) string {
	if gce.IsOpsAgentUAPPlugin() {
		...
		return fmt.Sprintf("%s -plaintext -max-time 60 -d '{}' localhost:1234 plugin_comm.GuestAgentPlugin/Start", grpcurlExecutable)
	}
...
func StartOpsAgentViaUAPCommand(imageSpec string, config string) string {
	...
	if len(config) > 0 {
		if gce.IsWindows(imageSpec) {
			return fmt.Sprintf("echo '{%s}' | %s -plaintext -max-time 60 -d \"@\" localhost:1234 plugin_comm.GuestAgentPlugin/Start", config, grpcurlExecutable)
		}
		return fmt.Sprintf("%s -plaintext -max-time 60 -d '{%s}' localhost:1234 plugin_comm.GuestAgentPlugin/Start", grpcurlExecutable, config)
	}
	return fmt.Sprintf("%s -plaintext -max-time 60 -d '{}' localhost:1234 plugin_comm.GuestAgentPlugin/Start", grpcurlExecutable)
}
...
func StopCommandForImage(imageSpec string) string {
	if gce.IsOpsAgentUAPPlugin() {
		...
		return fmt.Sprintf("%s -plaintext -max-time 60 -d '{}' localhost:1234 plugin_comm.GuestAgentPlugin/Stop", grpcurlExecutable)
	}
...
func GetUAPPluginStatusForImage(imageSpec string) string {
	...
	return fmt.Sprintf("%s -plaintext -max-time 60 -d '{}' localhost:1234 plugin_comm.GuestAgentPlugin/GetStatus", grpcurlExecutable)
