...
func runForEachExporter(t *testing.T, testName string, exporterList []string, testBody func(t *testing.T, exporter string)) {
	t.Helper()
...
func TestThirdPartyApps(t *testing.T) {
	...
	for _, tc := range tests {
		...
		// runForEachExporter(t, testName, []string{"otlphttp", "googlecloudmonitoring"}, func(t *testing.T, exporter string) {
		// t.Parallel()
		t.Run(testName, func(t *testing.T) {
			...
			for attempt := 1; attempt <= 4; attempt++ {
				...
				var retryable bool
				retryable, err = runSingleTest(ctx, logger, vm, tc.app, tc.metadata, "otlphttp")
