...
func mainErr() error {
	...
	_, err = gce.RunRemotely(ctx, logger, vm, "", `Set-ItemProperty -Path "HKCU:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\MusNotification.exe" -Name "Debugger" -Value "cmd.exe"`)
	if err != nil {
		return err
	}
