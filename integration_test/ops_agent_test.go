...
func testAgentCrashRestart(ctx context.Context, t *testing.T, logger *logging.DirectoryLogger, vm *gce.VM, processNames []string, otherProcessNames []string, livenessChecker func(context.Context, *log.Logger, *gce.VM) error) {
	...
	// 	t.Fatal(err)
	// }
	if processName == "fluent-bit" {
		if _, err := gce.RunRemotely(ctx, logger.ToMainLog(), vm, "", "sudo service google-cloud-ops-agent-fluent-bit restart || sudo systemctl restart google-cloud-ops-agent-fluent-bit"); err != nil {
			t.Fatal("failed to restart")
		}
	}

	if processName == "otelopscol" {
		if _, err := gce.RunRemotely(ctx, logger.ToMainLog(), vm, "", "sudo service google-cloud-ops-agent-opentelemetry-collector restart || sudo systemctl restart google-cloud-ops-agent-opentelemetry-collector"); err != nil {
			t.Fatal("failed to restart")
		}
	}
