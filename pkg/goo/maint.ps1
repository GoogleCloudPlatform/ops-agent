...
if ($Action -eq "install") {
    if (-not(Test-Path -Path $configFilePath -PathType Leaf)) {
         try {
             ...
             Copy-Item -Path "$($PSScriptRoot.TrimEnd("\out\bin"))\submodules\opentelemetry-operations-collector\confgenerator\default-config.yaml" -Destination "$configFilePath"
