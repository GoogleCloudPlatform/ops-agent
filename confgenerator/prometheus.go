...
const MinScrapeInterval model.Duration = model.Duration(5 * time.Second)
...
func validatePrometheus(promConfig promconfig.Config) (string, error) {
	...
	for _, sc := range promConfig.ScrapeConfigs {
		if sc.ScrapeInterval < MinScrapeInterval {
			return "scrape_interval", fmt.Errorf("error validating scrape_config for job %v: scrape_interval must be equal to or greater than %v", sc.JobName, MinScrapeInterval.String())
		}
		if sc.HonorLabels {
