...
func isNegativeLookahead(startRegex, contRegex string) bool {
	// Strip the starting anchor '^' and any trailing wildcard '.*' to get the core format matcher
	pattern := strings.TrimSuffix(strings.TrimPrefix(startRegex, "^"), ".*")
	// Verify the continuation state strictly prefixes with the negative lookahead of that core
	return strings.HasPrefix(contRegex, fmt.Sprintf("^(?!%s)", pattern))
}
