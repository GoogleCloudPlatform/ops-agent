...
func GenerateFiles(input, service, logsDir, stateDir, outDir string) error {
	...
	case "otel":
		otelConfig, err := uc.GenerateOtelConfig()
		if err != nil {
			return fmt.Errorf("can't parse configuration: %w", err)
		}
		// Make sure the output directory exists before generating configs.
...
func GenerateFilesFromUnifiedConfigWindows(unified *UnifiedConfigWindows, service, logsDir, stateDir, outDir string) error {
	switch service {
	case "fluentbit":
		mainConfig, parserConfig, err := GenerateFluentBitConfigsFromUnifiedConfig(unified.Logging, logsDir, stateDir)
		if err != nil {
			return fmt.Errorf("can't parse configuration: %w", err)
		}
		// Make sure the output directory exists before generating configs.
	...
	case "otel":
		otelConfig, err := GenerateOtelConfigFromUnifiedConfig(unified.Metrics)
		if err != nil {
			return fmt.Errorf("can't parse configuration: %w", err)
		}
		// Make sure the output directory exists before generating configs.
