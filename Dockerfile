...
FROM rockylinux:9 AS rocklinux9-build
...
FROM scratch AS rocklinux9
COPY --from=rocklinux9-build /tmp/google-cloud-ops-agent.tgz /google-cloud-ops-agent-centos-9.tgz
COPY --from=rocklinux9-build /google-cloud-ops-agent*.rpm /
